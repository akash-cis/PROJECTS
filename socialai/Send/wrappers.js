...
export const wrapPageElement = ({ element, props }) => {
  ...
  }else if(props.path.indexOf("/user-review/") !== -1 || props.path.indexOf("/reviews-lists/") !== -1){
    return <ApolloProvider client={apolloClientReview()}>{element}</ApolloProvider>
  } else {
...
const apolloClientReview = () => {
  return new ApolloClient({
    uri: "http://localhost:5000/review_graphql",
    cache: new InMemoryCache(),
    resolvers,
    typeDefs,
  })
