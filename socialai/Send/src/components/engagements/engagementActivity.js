...
import StarIconWhite from "../../../static/icons/StarIconWhite.svg"
import DownloadContractTemplate from "../../../static/icons/DownloadContractTemplate.svg"
...
import ReviewTemplate from "../life-events/reviewTemplate"
...
import ReviewSendConfirmationModal from "./reviewSendConfirmationModal"
...
const LeadActivityView = ({ queryString, lead, onBackClick, onFullScreen }) => {
  ...
  const token = jwt.sign({ id: user?.id, company: user?.company?.id }, 'shhhhh', { expiresIn: 31556926 })
  ...
  const [createMmsMessage] = useMutation(CREATE_MMS_MESSAGE)
  ...
  useEffect(() => {
    if (company && company.facebookLink && company.facebookLink !== "" || company && company.googleLink && company.googleLink !== "") {
      setButtonState(true);
  ...
  const onClickMmsHandler = (content, lead, userId, image_url) => {
    if (!isEmpty(content)) {
      let message = {
        leadId: lead?.id,
        userId: userId,
      ...
      setSelectedLead(_selectedLead)
      text.current.value = ""
      const res = createMmsMessage({
        variables: {
          ...message,
        },
      }).then(res => {
        if (res?.data?.createMmsMessage?.statusCode == 200) {
          let resMessage = res?.data?.createMmsMessage?.leadMessage || {}
          const id = parseInt(resMessage?.id)
          createMessageLog({
            variables: {
              messageId: id,
          ...
          message.error(res?.data?.createMmsMessage?.message)
        }
      })
    }
  }
  ...
  return (
    <>
      ...
      <ContainerCustom id={"conversation-cntr"} auto>
          ...
          <SelectedDealCntr>
            <ContentBodyConversationsWrap height={fullScreen ? "88vh" : "64vh"}>
                                      ...
                                      <MessageSentBox>
                                        <MessageSentRow>
                                          <LinkifyText>
                                            ...
                                            {/* <div className={LinkifyText}><Linkify>{c?.content || ""}</Linkify></div> */}
                                          <Icon
                                            type={
                                              c?.type == "new"
                                                ? "loading"
              ...
              <OptionContainer>
                ...
                <Row gutter={[0, 4]}>
                  <Col xxl={19} lg={10} xl={16} sm={24} md={24}>
                    <InputContainer>
                      <InputTextWrap
                        id="text"
                        ref={text}
                  ...
                  <Col xxl={5} xl={9} lg={8} sm={24} md={24}
                    style={{
                  ...
                  >
                    ...
                    {buttonState ? <TooltipButton
                      tooltip="Send Review SMS"
                      type="secondary"
                      disabled={!selectedLead}
                      shape="circle"
                      onClick={() => handleSendReviewClick()}
                      component={StarIconWhite}
                      alt="Send-review-sms"
                    /> : <></>}
                    {/* <TooltipButton
                      tooltip="Review SMS Template"
                      type="secondary"
                      disabled={!selectedLead}
                      shape="circle"
                      onClick={() => {handleReviewTemplateUploadClick()}}
                      component={DownloadContractTemplate}
                      alt="review-sms-template"
                    /> */}
                    <TooltipButton
        ...
        {openSendReviewModal && (
          <ReviewSendConfirmationModal
            selectedLead={selectedLead}
            user={user}
            openModal={openSendReviewModal}
            setModalVisible={setSendReviewVisible}
            sendSendReview={() => {

              // token = jwt.sign(
              //   { id: user?.id, company: user?.company?.id },
              //   "secret",
              //   { expiresIn: 31556926 },
              //   (err, token) => {
              ...
              onClickHandler(
                `Please Give us a review via this link. http://localhost:8000/user-review/${token}`,
                selectedLead,
                user?.id,
                // 'https://images.ctfassets.net/t21gix3kzulv/2VCzvnu6f2Rn7GgGdSac1B/ef81004b8e07b6af376608aa1745a0cb/templates-hero-desktop.jpg',
              )
            }
            }
            />
        )}
        {openSetReviewTemplateUploadModal && (
          <ReviewTemplate
            sources={sourcesData?.leadSources || []}
            isVisible={openSetReviewTemplateUploadModal}
            setIsVisible={setReviewTemplateUploadVisible}
          />
        )}
        {openTemplateModal && (
          <TemplateDrawer
