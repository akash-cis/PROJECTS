...
const CompanyProfileForm = ({ company, setCompany }) => {
  ...
  useEffect(() => {
    if ( company.googleLink == null || company.googleLink.length !== 0) {
      setCheck2(true)
    }
    if ( company.facebookLink == null || company.facebookLink.length !== 0) {
      setCheck1(true)
    }
  }, [company]);
  ...
  const handlefacebook = () => {
    ...
    if (check1) {
      ...
      updateCompanyField("facebookLink", "")
    }
  }
  ...
  const handlegoogle = () => {
    ...
    if (check2) {
      console.log("empty google link");
      updateCompanyField("googleLink", "")
    }
  }


  return (
    <StatusContext.Provider value={{check1, check2}}>
    <div style={{ height: "90%", overflow: "auto", padding: 24 }}>
      ...
      <Input
        placeholder="Enter Google/Bing/Other online maps link"
      ...
      <div style={{ display: 'flex' }}>
        <Label style={{ marginRight: '1rem' }}>FaceBook Review Link</Label>
        <Checkbox checked={check1} onChange={() => handlefacebook()} />
      </div>
      {check1 ? <Input
        placeholder="Enter your Company facebook-review link"
        value={company.facebookLink}
        onChange={e => updateCompanyField("facebookLink", e.target.value)}
      /> : ""}
      <br />
      <br />
      <div style={{ display: 'flex' }}>
        <Label style={{ marginRight: '1rem' }}>Google Review Link</Label>
        <Checkbox checked={check2} onChange={() => handlegoogle()} />
      </div>
      {check2 ? <Input
        placeholder="Enter your Company google-review link"
        value={company.googleLink}
        onChange={e => updateCompanyField("googleLink", e.target.value)}
      /> : ""}
      <br />
      <br />
    ...
    <InfoCntr>
      ...
      <InfoSection>
        ...
        >
          ...
          { company.facebookLink == null || company.facebookLink.length === 0 ? "" :
            <Descriptions.Item label={"Facebook Review URL"}>
              {company.facebookLink}
            </Descriptions.Item>}
          { company.googleLink == null || company.googleLink.length === 0 ? "" :
            <Descriptions.Item label={"Google Review URL"}>
              {company.googleLink}
            </Descriptions.Item>}
